...
resumo_filial["STATUS"] = resumo_filial["% OCUPAÇÃO"].apply(status_ocupacao)
resumo_filial = resumo_filial.sort_values("ESTOQUE", ascending=False)

top5_filiais = resumo_filial.nlargest(5, "ESTOQUE").copy()
...
cond_df["STATUS"] = cond_df["%"].apply(status_ocupacao)
...
cd_fab_resumo["STATUS"] = cd_fab_resumo["% OCUPAÇÃO"].apply(status_ocupacao)
cd_fab_resumo = cd_fab_resumo.rename(
    columns={"tipo_unidade": "VISÃO"}).sort_values("ESTOQUE", ascending=False)

alertas = resumo_filial[resumo_filial["% OCUPAÇÃO"] >=
                        75].copy().sort_values("% OCUPAÇÃO", ascending=False)
...
with a1:
    st.markdown("<div class='panel'><div class='panel-title'>TOP 5 FILIAIS POR ESTOQUE</div>",
                unsafe_allow_html=True)

    top5_fmt = top5_filiais.rename(
        columns={"filial": "FILIAL", "tipo_unidade": "VISÃO"}).copy()
    top5_fmt["ESTOQUE"] = top5_fmt["ESTOQUE"].apply(fmt_num)
    top5_fmt["CAPACIDADE"] = top5_fmt["CAPACIDADE"].apply(fmt_num)
    top5_fmt["% OCUPAÇÃO"] = top5_fmt["% OCUPAÇÃO"].apply(fmt_pct)
    st.dataframe(top5_fmt[["FILIAL", "VISÃO", "ESTOQUE", "CAPACIDADE",
                 "% OCUPAÇÃO", "STATUS"]], use_container_width=True, hide_index=True)

    st.markdown("</div>", unsafe_allow_html=True)

with a2:
    st.markdown("<div class='panel'><div class='panel-title'>ALERTAS DE CAPACIDADE</div>",
                unsafe_allow_html=True)

    if alertas.empty:
        st.success("Nenhuma filial em nível de alerta.")
    else:
        alertas_fmt = alertas.rename(
            columns={"filial": "FILIAL", "tipo_unidade": "VISÃO"}).copy()
        alertas_fmt["ESTOQUE"] = alertas_fmt["ESTOQUE"].apply(fmt_num)
        alertas_fmt["CAPACIDADE"] = alertas_fmt["CAPACIDADE"].apply(fmt_num)
        alertas_fmt["% OCUPAÇÃO"] = alertas_fmt["% OCUPAÇÃO"].apply(fmt_pct)
        st.dataframe(alertas_fmt[["FILIAL", "VISÃO", "ESTOQUE", "CAPACIDADE",
                     "% OCUPAÇÃO", "STATUS"]], use_container_width=True, hide_index=True)
...
with c2:
    ...
    fig2.update_layout(
        ...
        height=max(360, len(grafico_bar) * 34),
        xaxis_title="% ocupação",
        yaxis_title=""
    )
    st.plotly_chart(fig2, use_container_width=True)

    tabela_bar = grafico_bar.rename(
        columns={"filial": "FILIAL", "tipo_unidade": "VISÃO"}).copy()
...
with r1:
    ...
    st.dataframe(cond_show[["CONDIÇÃO", "GRUPO", "OCUPAÇÃO", "CAPACIDADE",
                 "%", "STATUS"]], use_container_width=True, hide_index=True)
...
st.dataframe(cdfab_show[["VISÃO", "ESTOQUE", "CAPACIDADE", "% OCUPAÇÃO",
             "STATUS"]], use_container_width=True, hide_index=True)

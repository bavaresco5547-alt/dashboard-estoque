...
BOVINO_KEYWORDS = [
    "BOVINO", "BOVINA", "CORTE BOVINO", "CORTE BOVINA", "CARNE BOVINA", "BOV",
    "BOI", "VACA", "NOVILHO", "GARROTE", "TOURO", "DIANTEIRO BOV", "TRASEIRO BOV",
    "MIUDOS BOV", "FIGADO BOV", "FIGADO", "RABO BOV", "BUCHO BOV", "BUCHO",
    "MOCOTO", "DOBRADINHA", "GLOTE", "CORACAO", "CORAÇÃO", "PATINHO", "ACEM",
    "ACÉM", "COXAO MOLE", "COXÃO MOLE", "COXAO DURO", "COXÃO DURO", "CONTRA FILE",
    "CONTRA FILÉ", "FILE MIGNON", "FILÉ MIGNON", "FRALDA", "PICANHA", "MAMINHA",
    "LAGARTO", "CUPIM", "PALETA BOV", "COSTELA BOV", "JERKED BEEF", "CHARQUE",
    "CARNE SECA", "PONTA DE AGULHA", "GORDURA ABATE", "SUB PRODUTO", "SUB PRODUTOS"
]
SUINO_KEYWORDS = [
    "SUINO", "SUINA", "SUÍNO", "SUÍNA", "CORTE SUINO", "CORTE SUÍNO",
    "CARNE SUINA", "CARNE SUÍNA", "PORCO", "PERNIL SU", "LOMBO SU", "COSTELA SU",
    "PALETA SU", "BARRIGA SU", "TOUCINHO", "BACON", "COPA LOMBO", "PANCETA",
    "PERNIL", "LOMBO", "COSTELA SUINA", "COSTELA SUÍNA", "EMBUTIDO", "EMBUTIDOS",
    "LINGUICA", "LINGUIÇA", "LINGUICA TOSCANA", "LINGUIÇA TOSCANA", "CALABRESA",
    "MY PORK", "KIT FEIJOADA"
]
AVE_KEYWORDS = [
    "AVE", "AVES", "FRANGO", "GALINHA", "GALO", "COXA", "SOBRECOXA", "ASA",
    "MEIO DA ASA", "DRUMET", "PEITO DE FRANGO", "FILE DE FRANGO", "FILÉ DE FRANGO",
    "CORAÇÃO DE FRANGO", "MOELA", "PE DE FRANGO", "PÉ DE FRANGO"
]
PET_KEYWORDS = ["PET", "RACAO PET", "RAÇÃO PET",
                "DOG", "CAT", "CAES", "CÃES", "GATOS"]
OUTROS_FORCE_KEYWORDS = [
    "VEGETAIS", "BATATA", "EMBALAGEM", "CAIXA", "PLASTICO", "PLÁSTICO", "ETIQUETA",
    "PAPELAO", "PAPELÃO", "BANDEJA", "SACO", "FILME", "LACRE", "TRIPA", "TRIPA SECA",
    "ENVOLTORIO", "ENVOLT."
]
...
def prepare_data(
    ...
    detail["tipo_estoque"] = detail.apply(
        lambda row: produto_para_condicao.get(str(row["id_produto"]).strip(
        ), "") if row["tipo_estoque"] == "" else row["tipo_estoque"],
        axis=1
    )
    detail["tipo_estoque"] = detail.apply(
        lambda row: classify_group(
            row["grupo"], group_map) if row["tipo_estoque"] == "" else row["tipo_estoque"],
        axis=1
    )
    ...
    detail["corte_animal"] = detail.apply(
        lambda row: produto_para_corte.get(str(row["id_produto"]).strip(
        ), "OUTROS") if row["corte_animal"] in ["", "OUTROS"] else row["corte_animal"],
        axis=1
    )
    detail["corte_animal"] = detail.apply(
    ...
    # adiciona a capacidade total da filial vinda do CSV
    cap_total = capacities[["filial_norm",
                            "capacidade_total"]].drop_duplicates()
    resumo = resumo.merge(cap_total, how="left", on="filial_norm")

    resumo["capacidade_tipo"] = resumo["capacidade_tipo"].fillna(0)
    resumo["capacidade_total"] = resumo["capacidade_total"].fillna(0)
    ...
    resumo = resumo[
        [
            ...
            "peso_total", "quantidade_total", "capacidade_tipo", "capacidade_total", "ocupacao_percentual"
        ]
    ]
